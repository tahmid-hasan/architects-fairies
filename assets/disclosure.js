class Disclosure extends HTMLElement{
  ...
  updateFormListeners(){
    window.on("resize.Disclosure",theme.utils.debounce(100,()=>this.updateFormPosition())),
  ...
  showFormWhenClick(){
    this.toggle.on("click.Disclosure keydown.Disclosure",t=>{
      "keydown"===t.type&&"Enter"!==t.key||(t="true"===this.toggle.getAttribute("aria-expanded"),
      this.toggleFormDisplay(!t))
    })
  }
  ...
  setOptionOnClick(){
    this.links.on("click.Disclosure keydown.Disclosure",t=>{
      if("Enter"===t.key)this.toggle.focus();else if("keydown"===t.type)return;
      const e=t.target;
      var t=e.dataset.value,s=e.innerHTML;
      this.toggleFormDisplay(!1),this.current_option.innerHTML=s,this.links.forEach(t=>t.setAttribute("aria-current",!1)),
      e.setAttribute("aria-current",!0),
      this.input.value=t,this.input.trigger("change")
    })
  }
  toggleFormDisplay(t){
  ...
  updateFormPosition(){
    var t=this.toggle.offsetHeight,e=t+this.toggle_and_form_gap,s=this.closest("[class^=off-canvas]"),
    ...
    t?r-=s:o&&(r-=i),
    this.form.style.maxHeight=r+"px",
    t?(s=theme.utils.getHiddenElHeight(this.form,!1)+this.toggle_and_form_gap,
    this.form.style.top=`-${s}px`):this.form.style.top=e+"px"
  }
  optionChangeCallback(){this.input.on("change.Disclosure",t=>{const e=t.target;"url-redirect"===this.type?window.location.href=e.value:"localization"===this.type&&e.closest("form").submit()})}}customElements.define("disclosure-root",Disclosure);